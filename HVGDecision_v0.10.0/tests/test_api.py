...
def test_legacy_budget_recommends_smallest_eligible_plateau_not_peak(tmp_path: Path, monkeypatch):
    study = hd.setup_reference_query(
        synthetic_adata(),
        mode="within_domain",
        batch_key="donor",
    ...
    # Legacy 0.9.1 already chose the smaller biology-eligible plateau panel.
    # The original fixture incorrectly equated observed peak with recommendation.
    assert result.observed_peak_n_hvg == 6
    assert result.best_n_hvg == 4
    assert result.harmful_genes == []
    assert result.final_n_hvg == 4
    assert result.adata.n_vars == 10  # full gene space is retained
    assert int(result.adata.var["highly_variable"].sum()) == 4
...
def test_external_hvg_headerless_txt_keeps_first_gene(tmp_path: Path):
    path = tmp_path / "hvg.txt"
    path.write_text("GENE_A\nGENE_B\nGENE_C\n", encoding="utf-8")
    normalized = api._read_external_hvg_input(
        path,
        gene_column="gene",
        rank_column="gene_rank",
    ...
    assert normalized.sort_values("gene_rank")["gene"].tolist() == [
