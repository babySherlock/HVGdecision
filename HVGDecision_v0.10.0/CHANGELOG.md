...
- Added `refine` and `audit_design` for the current multi-donor workflow.
- Added supported-design identifiability, weighted replication adequacy and automatic routing at w >= 0.75.
- Vendored Generic cell-level V1 and donor-replicate v2.0 notebook engines with source hashes and regression fixtures.
- Defaulted to an independently fitted Seurat v3 batch-aware 2000-gene panel; external ordered panels bypass fitting.
- Preserved zero-removal outcomes, biological protections and original panel membership; no fixed removal count or replenishment.
- Added full gene-level decisions, per-gate failures, routing audits, manifests and retained-count AnnData export.
- Made raw-count validation exhaustive and required ID alignment for external named tables; rejected cached count sources on a changed cell axis.
- Fixed donor-engine writable-array compatibility with pandas 3.
- Kept the 0.9 API explicitly legacy; moved its YAML CLI to `hvgdecision-legacy` and issued a migration warning from the public setup function.
- Preserved missing scores as missing when finite panels have identical values.
- Added current workflow examples, installation guidance, algorithm specifications and tests.
