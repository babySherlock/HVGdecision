...
from .continuous_workflow import refine


def parser() -> argparse.ArgumentParser:
    command = argparse.ArgumentParser(
        prog="hvgdecision",
        description="HVG refinement with automatic weighted A and continuous evidence fusion",
    )
...
def main() -> int:
    ...
    if args.command == 'audit':
        ...
        query_panel = pd.read_csv(args.query_hvg_table)['gene'].tolist() if args.query_hvg_table else None
        span = args.hvg_span if args.hvg_span == 'auto' else float(args.hvg_span)
        query_span = args.query_hvg_span if args.query_hvg_span in (None, 'auto') else float(args.query_hvg_span)
        result = refine(adata, **common, counts=args.counts, n_hvg=args.n_hvg, hvg_genes=panel,
                        ...
                        query=args.query, query_hvg_genes=query_panel, hvg_span=span, query_hvg_span=query_span)
        print(result)
