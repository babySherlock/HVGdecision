"""Current HVGDecision v0.14 multi-donor refinement workflow."""
...
from ._design_core import selection_mask
from .design import DesignConfig, DesignAuditResult, audit_design
from .fusion import UnifiedFusionConfig, CellGateConfig, DonorGateConfig, unified_margin_fusion
...
class RefinementResult:
    ...
    @property
    def routing(self):
        """Deprecated compatibility alias; use ``design`` for v0.14."""
        warnings.warn("result.routing is deprecated; use result.design", FutureWarning, stacklevel=2)
        return self.design
    ...
    def save(self, output_dir):
        ...
        self.design.audit.to_csv(path / 'design_audit.csv', index=False)
        self.design.coverage.to_csv(path / 'donor_celltype_coverage.csv', index=False)
        self.design.by_donor.to_csv(path / 'design_by_donor.csv', index=False)
        self.design.by_celltype.to_csv(path / 'design_by_celltype.csv', index=False)
...
def refine(adata, *, batch_key, label_key, counts=None, n_hvg=2000, hvg_genes=None,
           reference=None, protected_genes=None, mode='dual_evidence', design_config=None,
    ...
    restricts HVG selection and risk evidence to that cohort. Output
    ...
    Replication adequacy A is computed automatically as dataset-level audit metadata only.
    Both evidence components are evaluated for every candidate gene and fused on a common
    signed-margin scale. ``mode='auto'`` is a compatibility alias for ``dual_evidence``.
    ...
    if mode == 'continuous':
        warnings.warn('mode="continuous" is a deprecated v0.13 alias; using dual_evidence.', FutureWarning, stacklevel=2)
        mode = 'dual_evidence'
    if mode not in ('auto', 'dual_evidence'):
        raise ValueError('Use mode="dual_evidence" (or compatibility alias mode="auto").')
    ...
    fusion_config = fusion_config or UnifiedFusionConfig()
    ...
    for obj, cls in [(design_config, DesignConfig), (fusion_config, UnifiedFusionConfig), (cell_level_config, CellLevelConfig),
    ...
    design = audit_design(adata, batch_key=batch_key, label_key=label_key,
                          reference=reference, config=design_config)
    if design.summary['n_donors'] < 2 or design.summary['n_celltypes'] < 2:
        raise ValueError('Dual-evidence refinement requires at least two observed discovery donors and cell types; inspect audit_design first.')
    if not design.summary['identifiable']:
        message = 'Supported donor-cell-type design is not identifiable; interpretation is limited; the audit remains separate from gene-level fusion.'
    ...
    table, fusion_manifest = unified_margin_fusion(
        component_evidence['donor'], component_evidence['cell'],
        replication_adequacy=design.summary['replication_adequacy'], config=fusion_config)
    table['harmful'] = table.discovery_remove
    table['explicitly_protected'] = table.gene.isin(protected)
    table['decision_reason'] = np.where(table.harmful, 'removed_unified_margin_above_cutoff',
                                       'kept_unified_margin_at_or_below_cutoff')
    table.loc[table.explicitly_protected, 'decision_reason'] = 'kept_user_protected'
    actual = 'dual_evidence'
    engine_id = 'dual_evidence_centered_logmeanexp_v0_4'
    ...
    engine_hashes = {name: hashlib.sha256(Path(__file__).with_name(name).read_bytes()).hexdigest()
                     for name in ['_cell_level.py', '_donor_aware.py', '_design_core.py', 'design.py',
                                  '_signed_margin_core.py', 'fusion.py', 'refinement_workflow.py']}
    cohort_ids = adata.obs_names[mask].astype(str).tolist()
    manifest = dict(
        schema_version='0.14.0', software_versions=versions, engine_sha256=engine_hashes,
        engine_id=engine_id, requested_mode=mode, selected_mode=actual, design_audit=design.summary,
        component_selection_used=False, dataset_dependent_component_weight_used=False,
        replication_adequacy_source='computed_from_discovery_metadata',
    ...
    final.uns['hvgdecision'] = dict(
        ...
        gene_decisions=table.copy(), design_audit=design.audit,
    ...
    output = RefinementResult(final, table, design, updated_counts_audit, manifest,
