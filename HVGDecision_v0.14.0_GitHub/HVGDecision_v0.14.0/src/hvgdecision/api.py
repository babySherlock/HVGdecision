...
def _read_external_hvg_input(
    ...
    *,
    ...
    if isinstance(hvg, pd.DataFrame):
        ...
        # A very common local-HVG format is one gene per line with no header.
        # Support it explicitly so the first gene is not accidentally consumed
        # as a pandas column name. CSV/TSV tables with columns still use their
        # header normally.
        if lower_name.endswith((".txt", ".txt.gz")):
            raw = pd.read_csv(path, sep="\t", header=None, comment="#")
            if raw.shape[1] == 1:
                values = raw.iloc[:, 0].dropna().astype(str).str.strip()
                values = values.loc[values.ne("")]
                if len(values):
                    first = values.iloc[0].strip().lower()
                    header_aliases = {
                        "gene", "genes", "gene_id", "geneid",
                        "gene_symbol", "symbol", "feature", "features",
                    }
                    if first in header_aliases:
                        values = values.iloc[1:]
                frame = pd.DataFrame({gene_column: values.tolist()})
            else:
    ...
    if not isinstance(counts_layer, (str, type(None))) or looks_like_count_table_path(
        ...
        try:
            ...
            return CountSourceResult(
                ...
                obs_names=adata.obs_names.astype(str).copy(),
            )
...
def prepare_cross_domain_inputs(
    reference_adata,
    query_adata,
    *,
    reference_counts: Any = "auto",
    query_counts: Any = "auto",
    domain_key: str = "_hvgdecision_domain",
    reference_name: str = "reference",
    query_name: str = "query",
    index_unique: str = "__",
    return_audit: bool = False,
):
    """Prepare two independent AnnData objects for ``mode='cross_domain'``.

    Raw counts are detected independently in Reference and Query. The helper
    keeps only genes present in both raw-count matrices, constructs one AnnData
    whose ``X`` and ``layers['counts']`` are raw counts, and adds ``domain_key``
    with ``reference_name`` / ``query_name`` values.

    This helper never uses Query biological labels. It exists to remove the
    error-prone manual step of concatenating cross-dataset inputs that may keep
    raw counts in different AnnData locations.
    """
    import anndata as ad

    ref_result = find_raw_counts(reference_adata, reference_counts)
    qry_result = find_raw_counts(query_adata, query_counts)
    if not ref_result.valid:
        raise ValueError(
            "Reference raw counts could not be located. "
            f"Details: {ref_result.error}"
        )
    if not qry_result.valid:
        raise ValueError(
            "Query raw counts could not be located. "
            f"Details: {qry_result.error}"
        )

    def _matrix_and_genes(adata_obj, result: CountSourceResult):
        if result.matrix is not None:
            matrix = result.matrix
            genes = pd.Index(result.gene_names, dtype=str)
        else:
            matrix = count_matrix(adata_obj, result.source)
            genes = count_var_names(adata_obj, result.source).astype(str)
        matrix = matrix.tocsr() if sparse.issparse(matrix) else np.asarray(matrix)
        return matrix, pd.Index(genes, dtype=str)

    ref_matrix, ref_genes = _matrix_and_genes(reference_adata, ref_result)
    qry_matrix, qry_genes = _matrix_and_genes(query_adata, qry_result)
    if not ref_genes.is_unique:
        raise ValueError("Reference raw-count gene identifiers must be unique")
    if not qry_genes.is_unique:
        raise ValueError("Query raw-count gene identifiers must be unique")

    shared = ref_genes[ref_genes.isin(set(qry_genes))]
    if len(shared) < 3:
        raise ValueError(
            "Reference and Query share fewer than three raw-count genes; "
            "check gene identifier systems before cross-domain analysis"
        )
    ref_idx = ref_genes.get_indexer(shared)
    qry_idx = qry_genes.get_indexer(shared)
    ref_shared = ref_matrix[:, ref_idx]
    qry_shared = qry_matrix[:, qry_idx]

    ref_var = pd.DataFrame(index=pd.Index(shared, name="gene"))
    qry_var = pd.DataFrame(index=pd.Index(shared, name="gene"))
    ref_work = ad.AnnData(
        X=ref_shared.copy(),
    ...
    combined = ad.concat(
        [ref_work, qry_work],
        label=str(domain_key),
        keys=[str(reference_name), str(query_name)],
        index_unique=str(index_unique),
        join="inner",
        merge="same",
    )
    ...
    audit = pd.DataFrame(
        [
            {
                "role": "reference",
                "domain_value": str(reference_name),
                "raw_count_source": ref_result.location,
                "n_cells": int(reference_adata.n_obs),
                "n_raw_genes": int(len(ref_genes)),
                "n_shared_genes": int(len(shared)),
            },
            {
                "role": "query",
                "domain_value": str(query_name),
                "raw_count_source": qry_result.location,
                "n_cells": int(query_adata.n_obs),
                "n_raw_genes": int(len(qry_genes)),
                "n_shared_genes": int(len(shared)),
            },
        ]
    )
    combined.uns["hvgdecision_cross_domain_input"] = {
        "domain_key": str(domain_key),
        "reference_name": str(reference_name),
        "query_name": str(query_name),
        "reference_raw_count_source": ref_result.location,
        "query_raw_count_source": qry_result.location,
        "n_shared_genes": int(len(shared)),
        "query_biological_labels_used": False,
    }
    return (combined, audit) if return_audit else combined
